...
public interface IEmailService
{
    ...
    Task SendCoachInviteAsync(string toEmail, string inviteUrl, string teamName, string inviterName, string roleLabel);
}
...
public class EmailService : IEmailService
{
    ...
    public async Task SendCoachInviteAsync(string toEmail, string inviteUrl, string teamName, string inviterName, string roleLabel)
    {
        var host = _config["SMTP_HOST"];
        var from = _config["EMAIL_FROM"] ?? "noreply@protracker.app";
        ...
        if (string.IsNullOrWhiteSpace(host))
        {
            _logger.LogInformation("[CoachInvite] No SMTP configured. Invite link for {Email}: {Url}", toEmail, inviteUrl);
            return;
        }
        ...
        try
        {
            ...
            message.From.Add(MailboxAddress.Parse(from));
            message.To.Add(MailboxAddress.Parse(toEmail));
            message.Subject = $"You've been invited to coach {teamName} on ProTracker";
            message.Body = new BodyBuilder { HtmlBody = BuildCoachInviteHtml(inviteUrl, teamName, inviterName, roleLabel) }.ToMessageBody();
            ...
            _logger.LogInformation("[CoachInvite] Sent coach invite email to {Email}", toEmail);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CoachInvite] Failed to send email to {Email}. Invite link: {Url}", toEmail, inviteUrl);
        }
    }

    private static string BuildCoachInviteHtml(string inviteUrl, string teamName, string inviterName, string roleLabel) => $$"""
<!doctype html>
<html>
<body style="font-family: -apple-system, Segoe UI, Roboto, sans-serif; background:#0f172a; padding:32px;">
  <div style="max-width:480px; margin:0 auto; background:#ffffff; border-radius:16px; overflow:hidden;">
    ...
    <div style="padding:28px;">
      <h2 style="margin:0 0 12px; font-size:18px; color:#0f172a;">Join {{teamName}}'s coaching staff</h2>
      <p style="color:#475569; font-size:14px; line-height:1.6;">
        {{inviterName}} has invited you to join <strong>{{teamName}}</strong> on ProTracker as
        <strong>{{roleLabel}}</strong>. Accept to help manage the team's assessments, tasks and training.
      </p>
