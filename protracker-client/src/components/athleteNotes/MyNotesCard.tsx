...
import { NOTE_CATEGORY_STYLES, noteTitleOf, fmtNoteDate } from './noteMeta';
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="flex items-center gap-1 text-[11px] text-gray-400"><Lock size={11} /> Private</span>
      </div>
      ...
      {recent.length === 0 ? (
        ...
        <div className="space-y-2">
          {recent.map(n => (
            ...
            >
              <span className={clsx('text-[9px] px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0', NOTE_CATEGORY_STYLES[n.category])}>{n.category}</span>
              <span className="flex-1 min-w-0 text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{noteTitleOf(n)}</span>
              <span className="text-[10px] text-gray-400 flex-shrink-0">{fmtNoteDate(n.updatedAt)}</span>
            </button>
          ))}
        </div>
      )}
