...
export function PushPrompt() {
  ...
  async function enable() {
    ...
    if (promptedKey) localStorage.setItem(promptedKey, '1');
    setShow(false);
    if (result === 'subscribed') addToast('Notifications enabled', 'success');
    else if (result === 'denied') addToast('Notifications blocked in your browser settings', 'info');
    else if (result === 'error') addToast('Could not enable notifications', 'error');
  }
  ...
  return (
    <AnimatePresence>
      {show && (
        ...
        >
          <button onClick={dismiss} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 cursor-pointer" aria-label="Dismiss">
            <X size={16} />
          </button>
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-gray-900 dark:text-white">Turn on notifications</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Get alerts for new messages, tasks and announcements — even when ProTracker is closed.
              </p>
            </div>
          </div>
          <div className="flex gap-2 mt-3">
            <button onClick={dismiss} className="flex-1 py-2 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer">Not now</button>
