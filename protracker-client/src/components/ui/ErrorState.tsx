  ...
  return (
    <div className={clsx('flex flex-col items-center justify-center text-center', size === 'sm' ? 'py-8' : 'py-14')}>
      ...
      <h3 className="font-bold text-gray-700 dark:text-gray-300">
        {message ?? t('ui.failedToLoad', 'Failed to load {{thing}}.', { thing })}
      </h3>
      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('ui.tryAgain', 'Please try again.')}</p>
      {onRetry && (
        <button
          onClick={onRetry}
        ...
        >
          <RotateCw size={15} /> {t('ui.retry', 'Retry')}
        </button>
      )}
    </div>
  );
