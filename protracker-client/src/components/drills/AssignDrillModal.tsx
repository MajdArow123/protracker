...
export function AssignDrillModal({ isOpen, onClose, drill, players, lockedPlayerId }: Props) {
  ...
  async function handleAssign() {
    ...
    if (!pid) { addToast(t('tasks.selectPlayerError', 'Select a player'), 'error'); return; }
    try {
      await assign.mutateAsync({ id: drill.id, data: { playerId: pid, dueDate: dueDate || null, priority, note: note.trim() || null } });
      addToast(t('drills.assignedToast', 'Assigned "{{name}}"', { name: drill.name }), 'success');
      onClose();
    } catch (err) {
      addToast(err instanceof Error ? err.message : t('drills.failedAssign', 'Failed to assign drill'), 'error');
    }
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('drills.assignAsTask', 'Assign as Task')}>
      <div className="space-y-4">
        ...
        <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg px-4 py-3">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            <span className={clsx('px-2 py-0.5 rounded-full text-[10px] font-semibold', DIFFICULTY_BADGE[drill.difficulty])}>{L.difficulty(drill.difficulty)}</span>
            <span className={clsx('px-2 py-0.5 rounded-full text-[10px] font-semibold', CATEGORY_BADGE[drill.category])}>{L.category(CATEGORY_LABEL[drill.category])}</span>
          </div>
        </div>

        {!lockedPlayerId && (
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks.player', 'Player')}</label>
            <select
              value={playerId}
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('drills.noteOptional', 'Note (optional)')}</label>
          <textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)}
            placeholder={t('drills.notePlaceholder', 'Add coaching points — prepended to the drill instructions.')}
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button onClick={handleAssign} disabled={assign.isPending}>{assign.isPending ? t('tasks.assigning', 'Assigning…') : t('drills.assign', 'Assign')}</Button>
        </div>
      </div>
    </Modal>
  );
