  ...
  return (
    <div className="mt-2">
      <button onClick={() => setOpen(v => !v)} className="flex items-center gap-1 text-xs font-medium text-indigo-500 hover:underline cursor-pointer">
        <Dumbbell size={13} /> Recommended drills {open ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
      </button>

      {open && (
        <div className="mt-2 space-y-1.5">
          {matches.length === 0 ? (
            <p className="text-xs text-gray-400">No drills target {category} for this sport.</p>
          ) : matches.map(d => (
            <div key={d.id} className="flex items-center justify-between gap-2 rounded-lg bg-gray-50 dark:bg-gray-700/40 px-3 py-1.5">
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={clsx('px-1.5 py-0.5 rounded text-[10px] font-semibold', DIFFICULTY_BADGE[d.difficulty])}>{d.difficulty}</span>
                  {d.durationMinutes != null && <span className="inline-flex items-center gap-0.5 text-[10px] text-gray-500"><Clock size={9} /> {d.durationMinutes}m</span>}
                </div>
              </div>
              {canAssign && (
