...
export function CreateDrillModal({ isOpen, onClose, drill, defaultSportId }: Props) {
  ...
  async function handleSubmit() {
    if (!name.trim()) { addToast('A drill name is required', 'error'); return; }
    if (sportIds.length === 0) { addToast('Select at least one sport', 'error'); return; }
    const data = {
      name: name.trim(),
    ...
    try {
      if (isEdit && drill) { await updateDrill.mutateAsync({ id: drill.id, data }); addToast('Drill updated', 'success'); }
      else { await createDrill.mutateAsync(data); addToast('Drill created', 'success'); }
      onClose();
    } catch (err) {
      addToast(err instanceof Error ? err.message : 'Failed to save drill', 'error');
    }
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Drill' : 'Create Drill'} size="lg">
      <div className="space-y-4">
        <Input label="Name" placeholder="e.g. Passing Triangle" value={name} onChange={(e) => setName(e.target.value)} autoFocus />

        <div>
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
          <textarea rows={2} value={description} onChange={(e) => setDescription(e.target.value)}
            placeholder="Short summary of the drill…"
        ...
        {/* Sports (multi-select) */}
        <div>
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Sports</p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Input label="Duration (minutes)" type="number" placeholder="15" value={duration} onChange={(e) => setDuration(e.target.value)} />
          <Input label="Equipment" placeholder="Cones, Ball" value={equipment} onChange={(e) => setEquipment(e.target.value)} />
        </div>
        ...
        <Input label="Video URL (optional)" placeholder="https://youtube.com/…" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} />

        {/* Target stat categories */}
        <div>
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Targets improvement in</label>
          <div className="mt-1 flex flex-wrap items-center gap-1.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-2 py-1.5">
            ...
            <input value={tagInput} onChange={(e) => setTagInput(e.target.value)}
              ...
              onBlur={addTag}
              placeholder={targets.length ? '' : 'e.g. Passing, Ball Control'}
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button onClick={handleSubmit} disabled={saving}>{saving ? 'Saving…' : isEdit ? 'Save Changes' : 'Create Drill'}</Button>
        </div>
      </div>
    </Modal>
  );
