...
import { Plus, Pencil, Trash2, Lock, StickyNote } from 'lucide-react';
...
function NoteModal({ playerId, note, isOpen, onClose }: {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Note' : 'Add Private Note'}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea value={content} onChange={e => setContent(e.target.value)} rows={5} autoFocus placeholder="Private observation about this player…"
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <h3 className="font-bold text-gray-900 dark:text-white">Private Notes</h3>
      ...
      <p className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mb-4">
        <Lock size={12} /> Only coaches can see these notes — never shown to the athlete.
      </p>

      {isLoading ? (
        <p className="text-sm text-gray-500">Loading…</p>
      ) : notes.length === 0 ? (
        <EmptyState icon={<StickyNote size={32} />} title="No notes yet" description="Add a private note about this player." size="sm" />
      ) : (
      ...
      <ConfirmModal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)}
        ...
        title="Delete Note" message="Delete this private note?" confirmLabel="Delete" isLoading={deleteNote.isPending} />
    </div>
  );
}
