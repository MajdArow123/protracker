...
// Badge classes per confidence level (light + dark).
export function confidenceBadgeClass(c: EvidenceConfidence): string {
  switch (c) {
    ...
    case 'VeryHigh': return 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400';
  }
}
