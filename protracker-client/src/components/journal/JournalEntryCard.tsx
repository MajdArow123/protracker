...
import { MOOD_CONFIG, parseTags, formatEntryDate, autoTitle, dateKey } from './journalUtils';
...
export function JournalEntryCard({ entry, canManage, onEdit }: Props) {
  ...
  const title = entry.title || autoTitle(entry.mood, entry.entryDate);
      ...
      <div className="flex items-start gap-3">
        ...
        {canManage && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button onClick={() => onEdit?.(entry)} className="p-1.5 text-gray-400 hover:text-indigo-500 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" title="Edit"><Pencil size={15} /></button>
            <button onClick={handleDelete} className="p-1.5 text-gray-400 hover:text-red-500 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" title="Delete"><Trash2 size={15} /></button>
          </div>
        )}
      </div>
        ...
        <div className="mt-3 space-y-2">
          {entry.keyLearning && (
            <div className="flex gap-2 text-sm">
              ...
              <p className="text-gray-600 dark:text-gray-300"><span className="font-medium text-gray-700 dark:text-gray-200">Key learning: </span>{entry.keyLearning}</p>
            </div>
          )}
          {entry.tomorrowFocus && (
            <div className="flex gap-2 text-sm">
              <Target size={15} className="text-indigo-500 flex-shrink-0 mt-0.5" />
              <p className="text-gray-600 dark:text-gray-300"><span className="font-medium text-gray-700 dark:text-gray-200">Tomorrow: </span>{entry.tomorrowFocus}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-3">
        {tags.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {tags.map(t => (
              <span key={t} className="px-2 py-0.5 rounded-full text-[11px] bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300">#{t}</span>
            ))}
          </div>
        ) : <span />}
        {hasExtra && (
