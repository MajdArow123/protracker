...
export function EditableAvatar({ name, src }: { name: string; src?: string | null }) {
  ...
  const pick = (file: File | undefined | null) => {
    ...
    if (!ACCEPTED.includes(file.type)) {
      addToast(t('profile.avatar.invalidType', 'Use a JPEG, PNG, or WebP image'), 'error');
      return;
    }
    if (file.size > MAX_BYTES) {
      addToast(t('profile.avatar.tooLarge', 'Image is too large — maximum size is 5MB'), 'error');
      return;
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      addToast(t('profile.avatar.photoUpdated', 'Profile photo updated'), 'success');
      closePreview();
    } catch (err) {
      addToast(err instanceof Error ? err.message : t('profile.avatar.uploadFailed', 'Upload failed'), 'error');
    }
  };
  ...
  return (
    <>
      <div
        role="button"
        aria-label={t('profile.avatar.changePhoto', 'Change profile photo')}
      ...
      >
        ...
        <div className={clsx(
          ...
          <span className="text-[10px] font-semibold text-white">{dragOver ? t('profile.avatar.dropPhoto', 'Drop photo') : t('profile.avatar.changePhotoShort', 'Change photo')}</span>
        </div>
      ...
      {src && (
        <button
          onClick={() => remove.mutateAsync().then(() => addToast(t('profile.avatar.photoRemoved', 'Photo removed'), 'success')).catch(() => addToast(t('profile.avatar.removeFailed', 'Could not remove photo'), 'error'))}
        ...
        >
          <Trash2 size={11} /> {t('profile.avatar.removePhoto', 'Remove photo')}
        </button>
      )}

      {/* Preview before saving */}
      <Modal isOpen={!!pending} onClose={closePreview} title={t('profile.avatar.newPhotoTitle', 'New profile photo')} size="sm">
        {pending && (
          <div className="flex flex-col items-center gap-4 py-2">
            <img src={pending.previewUrl} alt={t('profile.avatar.previewAlt', 'Preview')} className="w-40 h-40 rounded-full object-cover ring-4 ring-indigo-500/30" />
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
              {t('profile.avatar.cropNote', 'Your photo will be cropped to a square and shown across ProTracker.')}
            </p>
            <div className="flex gap-2 w-full">
              ...
              >
                {upload.isPending ? <><Loader2 size={14} className="animate-spin" /> {t('profile.avatar.uploading', 'Uploading…')}</> : <><Upload size={14} /> {t('profile.avatar.savePhoto', 'Save Photo')}</>}
              </button>
            </div>
          </div>
        )}
