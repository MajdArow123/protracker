...
export function ShareProgressButton({ className }: { className?: string }) {
  ...
  async function share() {
    if (navigator.share) {
      try { await navigator.share({ title: 'My ProTracker progress', url }); return; } catch { /* cancelled */ }
    }
    try {
      await navigator.clipboard.writeText(url);
      setCopied(true);
      addToast('Progress link copied', 'success');
  ...
  return (
    ...
    >
      {copied ? <Check size={15} /> : <Share2 size={15} />} Share My Progress
    </button>
  );
}
