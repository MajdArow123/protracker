    ...
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-gray-900 dark:text-white">My Leagues</h2>
        </div>
        <Link to="/leagues" className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">View all</Link>
      </div>
          ...
          <Link key={l.id} to={`/leagues/${l.id}`}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                {l.sportName && <span className={clsx('px-1.5 py-0.5 rounded-full text-[9px] font-semibold', sportBadge(l.sportName))}>{l.sportName}</span>}
                <span className="text-[10px] text-gray-400">{l.teamCount} teams</span>
              </div>
            </div>
            <span className={clsx('px-2 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0', LEAGUE_STATUS_STYLES[l.status])}>{l.status}</span>
