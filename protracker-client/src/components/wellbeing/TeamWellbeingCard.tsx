  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-base font-bold text-gray-900 dark:text-white">Team Wellbeing</h2>
        {data.painAlerts > 0 && (
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-red-600 bg-red-100 dark:bg-red-900/30 rounded-full px-2 py-0.5">
            <ShieldAlert size={12} /> {data.painAlerts} alert{data.painAlerts === 1 ? '' : 's'}
          </span>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
        ...
        <div className="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-800 border-b border-gray-100 dark:border-gray-800">
          <div className="p-4 text-center">
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Checked in today</p>
          </div>
          <div className="p-4 text-center">
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Avg score today</p>
          </div>
          <div className="p-4 text-center">
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Pain alerts</p>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.painDuringRecovery && (
                      <span className="inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 flex-shrink-0">
                        <ShieldAlert size={10} /> Pain + recovery
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
                    {c
                      ? p.checkedInToday
                        ? `Checked in today${c.hasPain ? ` · pain: ${c.painArea || 'reported'}` : ''}`
                        : `Last: ${new Date(c.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
                      : 'No check-ins yet'}
                  </p>
                </div>
