    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h2 className="font-bold text-gray-900 dark:text-white">Wellbeing — Last 30 Days</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">{checkins.length} check-in{checkins.length === 1 ? '' : 's'}</p>
          </div>
        </div>

        {checkins.length === 0 ? (
          <EmptyState icon={<HeartPulse size={32} />} title="No check-ins yet"
            description="This athlete hasn't submitted any daily wellbeing check-ins." size="sm" />
        ) : (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-5">
              <StatTile icon={<Smile size={13} />} label="Feeling" value={data?.avgFeeling != null ? data.avgFeeling.toFixed(1) : '—'} suffix="/5" />
              <StatTile icon={<Zap size={13} />} label="Energy" value={data?.avgEnergy != null ? data.avgEnergy.toFixed(1) : '—'} suffix="/5" />
              <StatTile icon={<Moon size={13} />} label="Sleep" value={data?.avgSleep != null ? data.avgSleep.toFixed(1) : '—'} suffix="/5" />
              <StatTile icon={<HeartPulse size={13} />} label="Avg Score" value={data?.avgScore != null ? data.avgScore.toFixed(1) : '—'} suffix="/10" />
            </div>
            <LineChartWrapper
              data={chartData}
              series={[{ key: 'score', name: 'Wellbeing', color: '#6366f1' }]}
              height={260}
              focusedKey={null}
      ...
      {painReports.length > 0 && (
        <div className="rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50/50 dark:bg-red-900/10 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-bold text-gray-900 dark:text-white">Pain Reports</h3>
            ...
            {painReports.slice(0, 8).map(c => (
              <div key={c.id} className={clsx('rounded-xl border p-3 bg-white dark:bg-gray-900', 'border-red-100 dark:border-red-900/40')}>
                <div className="flex items-center justify-between gap-2 mb-0.5">
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{c.painArea || 'Pain reported'}</p>
