...
import type { RatedSessionPoint, SessionType } from '../../types';
...
function ExpandedResponses({ sessionId }: { sessionId: number }) {
  ...
  if (isLoading) return <p className="text-xs text-gray-400 px-3 py-2">Loading responses…</p>;
  if (!data || data.responses.length === 0) return <p className="text-xs text-gray-400 px-3 py-2">No responses yet.</p>;
  ...
  return (
    <div className="px-3 py-2 space-y-2 bg-gray-50/60 dark:bg-gray-800/30 rounded-b-xl">
      <div className="grid grid-cols-3 gap-2 text-center text-[11px] text-gray-500 pb-1">
        <div><span className="font-bold text-gray-900 dark:text-white">{data.averageRating?.toFixed(1) ?? '—'}</span> avg rating</div>
        <div><span className="font-bold text-gray-900 dark:text-white">{data.averageDifficulty?.toFixed(1) ?? '—'}</span> difficulty</div>
        <div><span className="font-bold text-gray-900 dark:text-white">{data.respondedCount}/{data.teamPlayerCount}</span> rated</div>
      </div>
      {data.responses.map(r => (
        <div key={r.id} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2.5">
          ...
          <div className="flex gap-3 mt-1 text-[11px] text-gray-500">
            <span>Energy {r.energyBefore}→{r.energyAfter}</span>
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-gray-500">
            {fmtDate(point.startTime)} · {TYPE_LABEL[point.sessionType]} · {point.respondedCount} rated
          </div>
        </div>
  ...
  return (
    <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-4">
        <MessageSquare size={16} className="text-indigo-400" /> Session Feedback
      </h3>

      {/* Analytics summary */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-3">
          <div className="flex items-center gap-1 text-xs text-gray-500 mb-1"><Star size={12} className="text-amber-500" /> Avg rating</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{analytics.overallAverageRating?.toFixed(1) ?? '—'}</div>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-3">
          <div className="flex items-center gap-1 text-xs text-gray-500 mb-1"><Activity size={12} className="text-sky-500" /> Avg difficulty</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{analytics.overallAverageDifficulty?.toFixed(1) ?? '—'}</div>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-3">
          <div className="flex items-center gap-1 text-xs text-gray-500 mb-1"><TrendingUp size={12} className="text-emerald-500" /> Best type</div>
          <div className="text-sm font-bold text-gray-900 dark:text-white truncate">{bestType ? TYPE_LABEL[bestType.sessionType] : '—'}</div>
        </div>
        <div className={clsx('rounded-xl border p-3', analytics.injuryFlagCount > 0 ? 'border-red-200 dark:border-red-900/50 bg-red-50/40 dark:bg-red-900/10' : 'border-gray-200 dark:border-gray-800')}>
          <div className="flex items-center gap-1 text-xs text-gray-500 mb-1"><AlertTriangle size={12} className="text-red-500" /> Injury flags</div>
      ...
      {analytics.byType.length > 1 && (
        <div className="mb-5">
          <p className="text-xs font-semibold text-gray-500 mb-2">Rating by session type</p>
          <div className="space-y-1.5">
            {analytics.byType.map(t => (
              <div key={t.sessionType} className="flex items-center gap-2">
                <span className="text-xs text-gray-600 dark:text-gray-300 w-20 flex-shrink-0">{TYPE_LABEL[t.sessionType]}</span>
