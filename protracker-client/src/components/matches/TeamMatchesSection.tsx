...
function LogMatchModal({ teamId, match, isOpen, onClose, onCreated }: {
  teamId: number; match: MatchResult | null; isOpen: boolean; onClose: () => void; onCreated: (m: MatchResult) => void;
}) {
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      setOurScore('0'); setOppScore('0'); setCompetition('League'); setVenue(''); setNotes('');
    }
  }, [isOpen, match]);
  ...
  async function handleSubmit() {
    ...
    // Map our/opponent score onto home/away per the toggle.
    const payload = {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Match' : 'Log Match'}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Input label="Our Score" type="number" min={0} value={ourScore} onChange={e => setOurScore(e.target.value)} />
          <Input label="Opponent Score" type="number" min={0} value={oppScore} onChange={e => setOppScore(e.target.value)} />
        </div>
        <div className="grid grid-cols-2 gap-3">
...
interface RatingRow { playerId: number; name: string; rating: number; goals: string; assists: string; yellowCards: string; redCards: string; minutesPlayed: string; }

function RateMatchModal({ match, players, isOpen, onClose }: {
  match: MatchResult | null; players: PlayerOption[]; isOpen: boolean; onClose: () => void;
}) {
  const { addToast } = useToast();
  ...
  useEffect(() => {
    ...
    setRows(players.map(p => {
      ...
      return {
        ...
        yellowCards: String(existing?.yellowCards ?? 0),
        redCards: String(existing?.redCards ?? 0),
        minutesPlayed: String(existing?.minutesPlayed ?? 90),
      };
    }));
  ...
  async function handleSave() {
    ...
    const ratings: RatingInput[] = rows.map(r => ({
      ...
      minutesPlayed: Number(r.minutesPlayed) || 0,
    }));
    try {
        ...
        {rows.map(row => (
          <div key={row.playerId} className="rounded-xl border border-gray-200 dark:border-gray-700 p-3">
            ...
            <div className="grid grid-cols-5 gap-2">
              {([['Goals', 'goals'], ['Assists', 'assists'], ['Yellow', 'yellowCards'], ['Red', 'redCards'], ['Mins', 'minutesPlayed']] as [string, keyof RatingRow][]).map(([label, key]) => (
                <div key={key}>
                  <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0.5">{label}</label>
                  <input type="number" min={0} value={row[key] as string}
                    onChange={e => update(row.playerId, { [key]: e.target.value } as Partial<RatingRow>)}
...
// ── Section ──────────────────────────────────────────────────────────────────
export function TeamMatchesSection({ teamId, players, isCoach }: { teamId: number; players: PlayerOption[]; isCoach: boolean; }) {
  ...
  // Keep the rate modal bound to the freshest match object after invalidation.
  ...
  return (
    <div>
              ...
              <div key={m.id} className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
                <div className="flex items-center gap-4 p-4">
                  <div className="text-center flex-shrink-0 w-20">
                    ...
                    <span className={clsx('text-[10px] font-bold px-2 py-0.5 rounded-full', rs.badge)}>{m.result}</span>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-[11px] uppercase tracking-wide text-gray-400 text-left">
                          ...
                          <th className="font-semibold pb-2 px-2">G</th>
                          <th className="font-semibold pb-2 px-2">A</th>
                          <th className="font-semibold pb-2 px-2">🟨</th>
                          <th className="font-semibold pb-2 px-2">🟥</th>
                          <th className="font-semibold pb-2 px-2">Min</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {m.ratings.map(r => (
                          <tr key={r.id} className="text-gray-700 dark:text-gray-300">
                            ...
                            <td className="px-2">{r.goals}</td>
                            <td className="px-2">{r.assists}</td>
                            <td className="px-2">{r.yellowCards}</td>
                            <td className="px-2">{r.redCards}</td>
                            <td className="px-2">{r.minutesPlayed}'</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {isCoach && (
        <>
          <LogMatchModal teamId={teamId} match={editMatch} isOpen={logOpen}
            onClose={() => { setLogOpen(false); setEditMatch(null); }}
            onCreated={(m) => { setLogOpen(false); setRateMatch(m); }} />
          <RateMatchModal match={rateMatchLive} players={players} isOpen={!!rateMatch} onClose={() => setRateMatch(null)} />
