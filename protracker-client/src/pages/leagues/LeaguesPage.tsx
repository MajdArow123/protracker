...
export function LeaguesPage() {
  ...
  return (
    <PageWrapper title="Leagues" actions={actions}>
      {/* Tabs */}
      <div className="flex gap-2 mb-5">
        {(['mine', 'browse'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'mine' ? 'My Leagues' : 'Browse Leagues'}
          </button>
        ))}
      </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setSport(null)}
              className={clsx('px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer',
                sport == null ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300')}>All</button>
            {sports.map(s => (
              <button key={s.id} onClick={() => setSport(s.id)}
                className={clsx('px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer',
                  sport === s.id ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300')}>{s.name}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={<Trophy size={30} />}
          title={tab === 'mine' ? 'No leagues yet' : 'No public leagues found'}
          description={tab === 'mine'
            ? (isCoach ? 'Create a league or register a team in one to see it here.' : "Your team isn't in any leagues yet.")
            : 'Try a different sport or search, or check back later.'}
          action={tab === 'mine' && isCoach ? { label: 'Create League', onClick: () => setCreateOpen(true) } : undefined}
        />
      ) : (
