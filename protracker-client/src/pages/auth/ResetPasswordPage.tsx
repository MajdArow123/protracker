...
export function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('auth.resetFailed', 'Could not reset your password. Please try again.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (validating) {
    return <Shell><div className="py-10 flex flex-col items-center gap-3"><Spinner size="lg" /><p className="text-sm text-gray-400">{t('auth.verifyingLink', 'Verifying reset link…')}</p></div></Shell>;
  }
  ...
  if (!tokenValid) {
    return (
      <Shell>
        ...
        <h1 className="text-xl font-black text-white">{t('auth.linkExpiredTitle', 'Link expired or invalid')}</h1>
        <p className="mt-2 text-sm text-gray-400 leading-relaxed">{t('auth.linkExpiredDesc', 'This password reset link has expired or has already been used.')}</p>
        ...
        <Link to="/login" className="mt-4 inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-indigo-400 transition-colors"><ArrowLeft size={15} /> {t('auth.backToLogin', 'Back to Login')}</Link>
      </Shell>
    );
  }
  ...
  if (done) {
    return (
      <Shell>
        ...
        <h1 className="text-xl font-black text-white">{t('auth.passwordResetSuccess', 'Password reset successfully!')}</h1>
        <p className="mt-2 text-sm text-gray-400 leading-relaxed">{t('auth.passwordResetSuccessDesc', 'You can now log in with your new password.')}</p>
  ...
  return (
    <Shell>
      <h1 className="text-xl font-black text-white">{t('auth.setNewPassword', 'Set new password')}</h1>
      {email && <p className="mt-1.5 text-sm text-gray-400">{t('auth.forEmail', 'for')} <span className="font-semibold text-gray-200">{email}</span></p>}

      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <div>
          <div className="relative">
            <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500" />
            <input type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t('auth.newPassword', 'New password')} autoComplete="new-password" required className={inputCls} />
          ...
          {password && (
            <div className="mt-2">
              ...
              <p className="text-xs text-gray-500">{t(strength.level === 0 ? 'auth.weak' : strength.level === 1 ? 'auth.fair' : 'auth.strong', strength.label)}{!strength.ok && t('auth.passwordStrengthHint', ' — use 8+ characters with an uppercase letter and a number')}</p>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="relative">
            <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500" />
            <input type={showConfirm ? 'text' : 'password'} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder={t('auth.confirmPassword', 'Confirm password')} autoComplete="new-password" required className={clsx(inputCls, confirm && (matches ? 'border-green-500/50' : 'border-red-500/50'))} />
          ...
          {confirm && !matches && <p className="mt-1.5 text-xs text-red-400">{t('auth.passwordsNoMatch', "Passwords don't match.")}</p>}
        </div>
