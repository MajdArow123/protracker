  ...
  return (
    <PageWrapper>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              {child.age != null && <span className="px-2.5 py-1 rounded-full bg-white/15 text-xs font-semibold">Age {child.age}</span>}
              ...
              {child.fitnessLevel != null && <span className="px-2.5 py-1 rounded-full bg-white/15 text-xs font-semibold">Fitness {child.fitnessLevel}/10</span>}
            </div>
          </div>
          <div className="text-center flex-shrink-0">
            <p className="text-3xl font-black">{child.overallAverage != null ? child.overallAverage.toFixed(1) : '—'}</p>
            <p className="text-[11px] text-white/70 uppercase tracking-wide">Overall Avg</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Skill radar */}
        <Card header={<span className="flex items-center gap-2"><Trophy size={16} className="text-indigo-500" /> Skill Profile</span>}>
          {radarData.length === 0 ? (
            <EmptyState title="No assessments yet" description="Scores will appear once the coach records an assessment." />
          ) : (
        ...
        {/* Wellbeing */}
        <Card header={<span className="flex items-center gap-2"><HeartPulse size={16} className="text-pink-500" /> Wellbeing</span>}>
          {child.wellbeing.length === 0 ? (
            <EmptyState title="No check-ins yet" description="Your child hasn't logged a wellbeing check-in." />
          ) : (
            <div>
              <div className="flex items-baseline gap-2 mb-3">
                <span className={clsx('text-3xl font-black', scoreColor(child.wellbeingScore))}>{child.wellbeingScore?.toFixed(1) ?? '—'}</span>
                <span className="text-xs text-gray-400">/ 10 latest</span>
              </div>
        ...
        {/* Injuries */}
        <Card header={<span className="flex items-center gap-2"><ShieldAlert size={16} className="text-red-500" /> Active Injuries</span>}>
          {child.injuries.length === 0 ? (
            <p className="text-sm text-gray-400 py-2">No active injuries — all clear. 🎉</p>
          ) : (
              ...
              {child.injuries.map((inj, i) => (
                <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={clsx('text-[10px] font-bold px-2 py-0.5 rounded-full', SEVERITY_CLR[inj.severity] ?? 'bg-gray-100 dark:bg-gray-800 text-gray-500')}>{inj.severity}</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-1">
                    {inj.recoveryStatus} · since {fmtDate(inj.injuryDate)}
                    {inj.expectedReturnDate ? ` · back ~${fmtDate(inj.expectedReturnDate)}` : ''}
                  </p>
                </div>
              ))}
            </div>
        ...
        {/* Upcoming sessions */}
        <Card header={<span className="flex items-center gap-2"><Calendar size={16} className="text-blue-500" /> Upcoming Sessions</span>}>
          {child.upcomingSessions.length === 0 ? (
            <p className="text-sm text-gray-400 py-2">No sessions scheduled.</p>
          ) : (
            <div className="space-y-2">
              {child.upcomingSessions.map((s, i) => (
                <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 p-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">{s.sessionType}</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-1 flex items-center gap-2 flex-wrap">
                    <span className="flex items-center gap-1"><Clock size={11} /> {fmtDateTime(s.startTime)} · {s.durationMinutes}min</span>
        ...
        {/* Tasks */}
        <Card className="lg:col-span-2" header={<span className="flex items-center gap-2"><CheckSquare size={16} className="text-emerald-500" /> Tasks</span>}>
          {child.tasks.length === 0 ? (
            <p className="text-sm text-gray-400 py-2">No tasks assigned.</p>
          ) : (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        <span className={PRIORITY_CLR[t.priority] ?? ''}>{t.priority}</span> · {t.category}
      ...
      <p className="text-center text-xs text-gray-400 flex items-center justify-center gap-1.5 mt-2">
        <Activity size={12} /> This is a read-only view shared with you by the coach.
      </p>
    </PageWrapper>
  );
}
