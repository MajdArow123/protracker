...
import { NOTE_CATEGORIES, NOTE_CATEGORY_STYLES, noteTitleOf, fmtNoteDate } from '../../components/athleteNotes/noteMeta';
...
export function AthleteNotesPage() {
  ...
  return (
    <PageWrapper title="My Notes" actions={actions}>
      <p className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mb-4 -mt-2">
        <Lock size={12} /> Private to you — your coach can never see these.
      </p>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No notes yet"
          description="Keep private notes on your training, nutrition, mindset and goals — just for you."
          action={{ label: 'Write your first note', onClick: openNew }}
        />
      ) : (
        <div className="space-y-2.5">
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className={clsx('text-[10px] px-2 py-0.5 rounded-full font-semibold', NOTE_CATEGORY_STYLES[n.category])}>{n.category}</span>
                      <span className="text-[11px] text-gray-400">{fmtNoteDate(n.updatedAt)}</span>
                    </div>
                    <p className="font-semibold text-gray-900 dark:text-white truncate">{noteTitleOf(n)}</p>
                    {!isOpen && (
          ...
          {visible.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-8">No {filter.toLowerCase()} notes yet.</p>
          )}
        </div>
      )}
